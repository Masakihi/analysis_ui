...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (svgRef.current) {
      ...
      // 対数軸のスケール設定
      const xScale = d3.scaleLog().domain([0.1, 10]).range([0, 400]);
      const yScale = d3.scaleLog().domain([1, 100000]).range([200, 0]);
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, minWidth: 600 }}>
                    {qList !== undefined && iList !== undefined ? 'hog' : <svg ref={svgRef} width={600} height={300}></svg>}
                  </Box>
                </Grid>
              </Grid>
            </Container>
