...
const App: React.FC = () => {
  ...
  const [graphDatasets, setGraphDatasets] = useState<DataPoint[]>();
  ...
  const handleScanSector = () => {
    ...
    formData.append('x_c', String(centerX));
    formData.append('y_c', String(centerY));
    formData.append('angle_range_list', '[90, 91],[73, 85],[27, 30],[-5, 2],[270, 285]');
  ...
  useEffect(() => {
    if (qList !== undefined && iList !== undefined) {
      const dataPoints: DataPoint[] = iList?.map((y, index) => ({ x: qList[index], y: y }));
      setGraphDatasets(dataPoints.slice(1));
      // setGraphDatasets(data);
    }
  }, [iList]);
  ...
  const handleAddData = () => {
    setDataList([...dataList, { min: 0, max: 0 }]);
  };
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, width: 400 }}>
                    {qList !== undefined && iList !== undefined ? (
                      <div className="App">
                        <LineChart width={500} height={600} data={graphDatasets}>
                  ...
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, width: 400, flexDirection: 'column' }}>
                    ...
                    <List>
                      {dataList.map((data, index) => (
                        <ListItem key={index}>
                          <WhiteFontTextField type="number" value={data.min} onChange={(e: ChangeEvent<HTMLInputElement>) => handleInputChange(index, 'min', Number(e.target.value))} />
                          ～
                          <WhiteFontTextField type="number" value={data.max} onChange={(e: ChangeEvent<HTMLInputElement>) => handleInputChange(index, 'max', Number(e.target.value))} />
