...
import BitmapCanvas from './bitmap';
...
const options = {
  scales: {
    x: {
      grid: {
        color: 'white', // X軸の補助線の色を白に設定
      },
      ticks: {
        color: 'white', // X軸のラベルの色を白に設定
      },
      scaleLabel: {
        display: true,
        labelString: 'X軸ラベル',
    ...
    y: {
      ...
      // type: "logarithmic",
      grid: {
        color: 'white', // Y軸の補助線の色を白に設定
      },
      ticks: {
        color: 'white', // Y軸のラベルの色を白に設定
      },
      scaleLabel: {
        display: true,
        labelString: 'Y軸ラベル',
...
const App: React.FC = () => {
  ...
  let chartInstance: Chart | null = null; // チャートインスタンス

  const initializeChart = () => {
    const canvas = document.getElementById('chartCanvas') as HTMLCanvasElement; // Canvas 要素を取得
    if (canvas) {
  ...
  const updateChart = () => {
    if (chartInstance) {
      ...
      chartInstance.update(); // チャートを再描画
    }
  };

  useEffect(() => {
    initializeChart(); // チャートの初期化
  }, []);
            ...
            <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <Grid container spacing={2}>
                <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, minWidth: 600, flexDirection: 'column' }}>
                    <Box>
                ...
                <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, minWidth: 600 }}>
                    {qList !== undefined && iList !== undefined ? 'hoge' : <canvas id="chartCanvas" width={400} height={400}></canvas>}
                  </Box>
                </Grid>
              </Grid>
            </Container>
