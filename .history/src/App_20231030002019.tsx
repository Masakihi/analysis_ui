...
const App: React.FC = () => {
  ...
  const [sectorGraphDatasets, setSectorGraphDatasets] = useState<DataPoint[]>();
  ...
  useEffect(() => {
    if (qList !== undefined && sectorIList !== undefined) {
      const dataPoints: DataPoint[] = sectorIList?.map((y, index) => ({ x: qList[index], y: y }));
      setSectorGraphDatasets(dataPoints.slice(1));
      // setGraphDatasets(data);
    }
  }, [sectorIList]);
              ...
              <Grid container spacing={2}>
                <Grid item xs={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Box display="flex" alignItems="center" justifyContent="center" sx={{ minHeight: 800, width: 400, flexDirection: 'column' }}>
                    ...
                    <Button variant="contained" onClick={handleAddData} startIcon={<AddIcon />}>
                      追加
                    </Button>
                    <List>
