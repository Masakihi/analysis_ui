...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (svgRef.current) {
      ...
      // 対数軸のスケール設定
      const xScale = d3.scaleLog().domain([0.1, 10]).range([100, 300]);
      const yScale = d3.scaleLog().domain([1, 100000]).range([100, 300]);

      // x軸を描画
      const xAxis = d3.axisBottom(xScale).ticks(5, '0.1s').tickSize(6).tickPadding(6);
      svg.append('g').attr('transform', `translate(0, 100)`).call(xAxis).selectAll('text').style('fill', 'white'); // ラベルの色を白くする
      ...
      svg.append('g').attr('transform', `translate(0, 100)`).call(yAxis).selectAll('text').style('fill', 'white'); // ラベルの色を白くする
                    ...
                    {qList !== undefined && iList !== undefined ? (
                      ...
                      <div className="App">
                        <LineChart width={800} height={400} data={data}>
                          ...
                          <XAxis dataKey="x" type="number" scale="log" domain={[1, 4]} />
                          <YAxis dataKey="y" type="number" scale="log" domain={[10, 10000]} />
                          <Tooltip />
                          <Legend />
